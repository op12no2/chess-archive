...
function sigmoid (x) {
  return 1.0 / (1.0 + Math.pow(10.0,-x/300.0));
}
...
function grunt () {
  ...
  while (changes) {
    ...
    for (var i=0; i < params.length; i++) {
      ...
      if (err < bestErr) {
        changes++;
        console.log('+',p.s, p.v0, tweak(p.a,p.i,0), 'epoch =', epoch, 'err =', err, 'changes =', changes);
        bestErr = err;
