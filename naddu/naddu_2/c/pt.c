...
static const PerftTest PERFT_POSITIONS[] = {
...
void perft_tests(void) {
  ...
  for (int i = 0; PERFT_POSITIONS[i].fen != NULL; i++) {
    const PerftTest *p = &PERFT_POSITIONS[i];

    position_fen(p->fen, NULL, 0);

    uint64_t result = perft(p->depth, 0);
    total_nodes += result;
    ...
    printf("%d %s %s %d %llu %llu %lld %s\n",
           i + 1, p->id, p->fen, p->depth,
           (unsigned long long)result, (unsigned long long)p->expected,
           (long long)(result - p->expected), res);
    fflush(stdout);
  }
  ...
  printf("passed %d failed %d nodes %llu elapsed %.2fs nps %llu\n",
         passed, failed, (unsigned long long)total_nodes,
         elapsed / 1000.0, (unsigned long long)nps);
  fflush(stdout);
}
